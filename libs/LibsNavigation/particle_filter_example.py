...
'''
def create_map(height, width, landmarks_count):
    landmarks      = numpy.zeros((landmarks_count, 2)).astype(numpy.float32)

    landmarks[:, 0]   = height*numpy.random.rand(landmarks_count)
    landmarks[:, 1]   = width*numpy.random.rand(landmarks_count)
